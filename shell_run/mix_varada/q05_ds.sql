...
UNION ALL       SELECT
      ...
      FROM
        varada.tpcds_300.store_returns
   )  salesreturns
...
UNION ALL       SELECT
      ...
      FROM
        varada.tpcds_300.catalog_returns
   )  salesreturns
   , varada.tpcds_300.date_dim
   , varada.tpcds_300.catalog_page
   WHERE ("date_sk" = "d_date_sk")
      AND ("d_date" BETWEEN CAST('2000-08-23' AS DATE) AND (CAST('2000-08-23' AS DATE) + INTERVAL  '14' DAY))
...
UNION ALL       SELECT
      ...
      FROM
        (varada.tpcds_300.web_returns
      LEFT JOIN varada.tpcds_300.web_sales ON ("wr_item_sk" = "ws_item_sk")
         AND ("wr_order_number" = "ws_order_number"))
   )  salesreturns
   , varada.tpcds_300.date_dim
   , varada.tpcds_300.web_site
   WHERE ("date_sk" = "d_date_sk")
      AND ("d_date" BETWEEN CAST('2000-08-23' AS DATE) AND (CAST('2000-08-23' AS DATE) + INTERVAL  '14' DAY))
...
FROM
  (
   SELECT
     'varada.tpcds_300.store channel' "channel"
